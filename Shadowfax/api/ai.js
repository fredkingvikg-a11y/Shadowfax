...
module.exports = async function(req, res) {
  ...
  return new Promise(function(resolve) {
    ...
    request.on('error', function(e) { res.status(500).json({ error: e.message }); resolve(); });
    request.setTimeout(25000, function() { request.destroy(); res.status(504).json({ error: 'Timeout' }); resolve(); });
